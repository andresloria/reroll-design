...
2. **Hero** — titular **"Tu [disciplina] que vende."**; el botón **↻ ¿Qué necesitás?** rerollea la disciplina (sitio web → redes → marca → tienda) y tira el dado. Antes cambiaba un adjetivo, que no decía nada; ahora el hero demuestra los tres servicios. Las palabras entran una por una (`.w`/`.wb` + `rrWordIn`, retrasos escalonados por JS). **Palabras cortas a propósito** (redes, marca, tienda): con "redes sociales" o "tienda online" el titular no cabía en una línea en escritorio y saltaba. Además el JS **mide la palabra que viene y anima el ancho** (`fijarAncho`), así el titular centrado se desliza en vez de brincar; si la palabra no cabe en el ancho disponible se deja fluir, para no provocar scroll horizontal en celular. **Cada disciplina lleva su artículo y su verbo** (`#rrArt` / `#rrWord` / `#rrVerb`) porque "Tus redes sociales que **venden**" no concuerda con el "Tu … vende" de las demás. **El titular rota solo cada 4,2 s** para que muestre los tres servicios sin que nadie toque nada; se pausa si el mouse está sobre el hero, salta el turno si la pestaña está oculta (`document.hidden`), no corre con `prefers-reduced-motion`, y **el clic en el dado lo detiene para siempre** (si el visitante tomó el control, la palabra no le cambia sola mientras lee). Compacto a propósito, para que asome "Proyectos, ya en línea" sin hacer scroll.
